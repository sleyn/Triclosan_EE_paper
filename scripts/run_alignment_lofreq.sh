...
cd ./$d
    ...
    do
    	...
    	java -jar $trimmdir/trimmomatic-0.36.jar PE \
			  -threads $thr \
				-phred33 \
				$r1 \
				$r2 \
				...
				$r2p \
				$r2s \
				ILLUMINACLIP:$aux/overrepresented.fasta:2:30:10 \
				SLIDINGWINDOW:4:15 \
				CROP:145 \
				HEADCROP:10 \
				MINLEN:65
...
do
	...
	do
	    ...
	    $bwadir/bwa mem \
			  -t $thr \
				-M \
				-R "$rg" \
				$aux/Escherichia_coli_BW25113.fna \
				$r1 \
				$r2 > $bwa_sam
	    java -jar $picarddir/picard.jar SortSam \
			  I=$bwa_sam \
				O=$bwa_bam \
				SO=coordinate \
				CREATE_INDEX=true
	...
	lofreq viterbi \
	  --ref $aux/Escherichia_coli_BW25113.fna \
		--keepflags \
		-o Sample_v.bam \
		Sample_i.bam
	...
	java -jar $gatkdir/GenomeAnalysisTK.jar \
	  -T BaseRecalibrator \
		-R $aux/Escherichia_coli_BW25113.fna \
		-I Sample_vi.bam \
	...
	lofreq call-parallel \
	  --pp-threads $thr \
		--call-indels \
		-f $aux/Escherichia_coli_BW25113.fna \
		-o variants.lofreq.vcf \
		Sample_r_bqsr.bam
	python3 $ijumpdir/ijump.py \
    -a Sample_r_bqsr.bam \
    -r "$aux"/Escherichia_coli_BW25113.fna \
    -g "$aux"/Escherichia_coli_BW25113.gff \
    -i "$aux"/ISTable_processing.txt
  cd ..
done
...
do
    ...
    i=${d%'/'}
    vcf="$d.vcf"
		ijumpreport=ijump_"$d".txt
    cp ./variants.lofreq.vcf ../Results/$vcf
		cp ./ijump_report_by_is_reg.txt ../Results/$ijumpreport
    cd ..
done
...
bgzip -d merged.gz
rm *.gz
java -jar $snpeffdir/snpEff/snpEff.jar \
  -ud 100 \
	-v Ecoli_BW25113 merged > ann_1.vcf
rm *.tbi
perl $aux/ann_pars.pl
python3 $aux/timeline.py \
  --gff $aux/Escherichia_coli_BW25113.gff \
	--ann ann.vcf \
	--rep $aux/r_coordinates.txt \
	--vcf_folder . \
	--out results.txt
python3 $ijumpdir/combine_results.py \
  -d . \
	-o ijump_combined_table.txt \
	-g $aux/Escherichia_coli_BW25113.gff
